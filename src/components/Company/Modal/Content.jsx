  ...
  return (
    <div className="Content_container">
      <div className="announcement_card">
        ...
        <p className="time">
          {props.formData.is_always_recruiting ? '상시모집' : props.formData.recruit_period_start}
        </p>
      </div>
      ...
      <div>
        우대사항:{' '}
        {props.formData.preferred_conditions && props.formData.preferred_conditions.join(', ')}
      </div>
      <div>
        기타조건: {props.formData.other_conditions && props.formData.other_conditions.join(', ')}
      </div>
      <Hr />
      ...
      <div className="box">
        <p>
          근무기간: {props.formData.work_duration}
          <span>{props.formData.근무기간협의 && '(협의 가능)'}</span>
        </p>
        <p>
          근무시간: {props.formData.근무시간}
          <span>{props.formData.근무시간협의 && '(협의 가능)'}</span>
        </p>
      </div>
      <div>
        근무요일: {props.formData.work_days && props.formData.work_days.join(', ')}
        <span>{props.formData.근무요일협의 && '(협의 가능)'}</span>
        <span>{props.formData.근무요일변동}</span>
      </div>
    </div>
  );
}
